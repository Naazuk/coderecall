...
<body>
  ...
  useEffect(() => {
    ...
    setTaskSolved(completedTasks.includes('Fetch'));
  }, []);

  // Load solved task state from localStorage on mount
  useEffect(() => {
    const completedTasks = JSON.parse(localStorage.getItem('completedTasks')) || [];
    setTaskSolved(completedTasks.includes('Fetch')); // Adjust this ID for each task
  }, []);
  ...
  const markAsSolved = () => {
    if (!taskSolved) {
      ...
      const completedTasks = JSON.parse(localStorage.getItem('completedTasks')) || [];
      completedTasks.push('Fetch'); // Adjust this ID for each task
  ...
  <script>
    function toggleMenu() {
      ...
      navLinks.classList.toggle('active');
        ...
        <div className="page" style={{padding:'6px'}}>
            <div className="left">
